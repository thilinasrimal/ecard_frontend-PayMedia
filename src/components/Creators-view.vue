  ...
  <div class="container" style="display: flex; flex-direction: column">
    <div class="table-container" v-show="tableActive">
      <div class="header" style="display: flex; margin-bottom: 20px; margin-top: 1rem">
        ...
        <b-button class="btn is-dark" @click="addCreator">
      ...
      <div class="table-container" id="dataTable">
        <table class="table table-hover table-striped" id="creatorTable" style="align-content: center">
          <thead>
          <tr>
          ...
          <tbody>
          <tr v-for="creator in creators" >
            <td>{{ creator.employee_no }}</td>
            <td>{{ creator.designation }}</td>
            <td>{{creator.name}}</td>
            <td>{{creator.username}}</td>
            <td>{{creator.contact_no}}</td>
            <td>{{creator.email}}</td>
            <td class="has-text-centered" v-if="creator.status==='active'">
              <b-tooltip label="Creator is Active"
                         position="is-left"
                         type="is-success"
            ...
            <td class="has-text-centered" v-if="creator.status==='not-active'">
              <b-tooltip label="Creator isn't Active"
                         position="is-left"
                         type="is-danger"
                         :active="active" target="">
            ...
            <td style="cursor: pointer" @click="edit(creator.id)">
    ...
    <CreatorCreate v-show="createCreatorActive" @goToBack="closeCreate"></CreatorCreate>
    <CreatorEdit
      v-show="creatorEditActive"
      v-if="editMode"
...
<script>
import CreatorCreate from "./Creator-Create.vue";
import CreatorEdit from "./Creator-Edit.vue";
...
export default {
  name: "Creators-view",
  props:'companyName',
  components: {CreatorEdit, CreatorCreate},
  data() {
    return {
      tableActive: true,
      createCreatorActive: false,
      creatorEditActive: false,
      creators: [],
      creatorData:'',
      editMode:false,
      active: true,
  ...
  methods: {
    ...
    edit(id) {
      axios.get(`${appSettings.$api_url}/api/Creator/${id}`)
        .then(response => {
          const creatorData = response.data.data.creator;

          const companyName = response.data.data.creator.creator_company_name;
          console.log('C:',companyName)
          this.$emit("getCreatorCompany",companyName)
          this.creatorData = creatorData
          this.tableActive = false;
          this.createCreatorActive = false;
          this.creatorEditActive = true;
          this.editMode=true;

          let u = response.data.data.creator.username
          axios.get(`${appSettings.$api_url}/api/Users/username/${u}`).then(
            res =>{
              const userPermissions = res.data.data.newUser.permissions;
              this.userPermissions = userPermissions
              this.$emit('getCreatorData', creatorData, userPermissions);
    ...
    sortTable(n) {
      ...
      table = document.getElementById("creatorTable");
      switching = true;
      dir = "asc";
      while (switching) {
        switching = false;
